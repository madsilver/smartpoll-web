...
public class kXMLElement
{
	...
	@SuppressWarnings("rawtypes")
	public kXMLElement(String tagName)
	{
		this(new Hashtable(), false, true, true);
		setTagName(tagName);
	}
...
//				writeEncoded(out, contents);
		...
		else if (children == null || children.isEmpty())
		{
			...
			out.println('>');
			Enumeration enu = enumerateChildren();

			while (enu.hasMoreElements())
			{
				kXMLElement child = (kXMLElement)(enu.nextElement());
...
//	}
	...
	public void addBooleanProperty(String key,
			boolean value) {
		addProperty(key, value ? 1 : 0);
	}
	
	public float getFloatProperty(String string) {
		return Float.parseFloat(getProperty(string, "0"));
	}
